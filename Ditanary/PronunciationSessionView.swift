...
class PronunciationManager: NSObject, ObservableObject, AVAudioPlayerDelegate, AVSpeechSynthesizerDelegate {
    ...
    func startRecording() throws {
        ...
        guard let recognitionRequest = recognitionRequest else {
            throw NSError(
                domain: "Ditanary.Pronunciation",
                code: 1,
                userInfo: [NSLocalizedDescriptionKey: "Không thể khởi tạo nhận diện giọng nói. Vui lòng thử lại."]
            )
        }
    ...
    func calculateSimilarity(target: String, input: String) -> PronunciationScore {
        ...
        return PronunciationScorer.score(target: target, input: input, averageConfidence: averageConfidence)
    }
}
